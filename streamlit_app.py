...
q_abs = eta_opt * q_solar * A_e * ones(size(t));
q_loss = U_loss * A_e * (T_e - T_inf);
q_gain = U_gain * A_e * (T_e - T_w);
q_superheat = f_superheater * m_dot * cp_s * (T_e - T_w);

plot(t/60, q_abs, 'b-', 'LineWidth', 2);
hold on;
plot(t/60, q_loss, 'r-', 'LineWidth', 2);
plot(t/60, q_gain, 'g-', 'LineWidth', 2);
plot(t/60, q_superheat, 'm-', 'LineWidth', 2);
xlabel('Tiempo (min)');
ylabel('Flujo de calor (W)');
title('Flujos de calor');
grid on;
legend('q_abs (absorbido)', 'q_loss (perdidas)', 'q_gain (al agua)', 'q_superheat (al vapor)');
...
fprintf('Flujo masico medio: %.2f g/h\\n', mean(m_dot(m_dot > 0)) * 3600 * 1000);
